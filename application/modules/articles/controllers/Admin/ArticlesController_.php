...
class Articles_Admin_ArticlesController extends MainAdminController {
    ...
    public function init() {
        ...
        $this->view->currentModul = SP.'articles'.SP.$lang.SP.$this->getRequest()->getControllerName();
    }
    ...
    public function indexAction() {
        ...
        $this->view->articles = Articles::getInstance()->fetchAll();
        
        $this->view->total = Articles::getInstance()->getCount("id_page='".$this->_id_page."'");
    ...
    public function addAction() {
        ...
        $this->view->currentModul = $curModul = SP.'articles'.SP.$lang.SP.$this->getRequest()->getControllerName();
        $this->view->lang = $lang;
        ...
        if ($this->_request->isPost()) {
            ...
            $id = Articles::getInstance()->addArticle($new);
            $item = Articles::getInstance()->find($id)->current();
            // иконка
            ...
            if ($img_name!='' && $img_source!='' ) {
                ...
                $small_img = DIR_PUBLIC.'pics/articles/'.$id.'_small.'.$ext;
            ...
            if ($img_name!='' && $img_source!='' && !$delete_img) {
                ...
                $big_img = DIR_PUBLIC.'pics/articles/'.$id.'_big.'.$ext;
        ...
        $this->view->trafficLighs = Articles::$traffic_light;
    ...
    public function editAction() {
        ...
        $this->view->currentModul = $curModul = SP.'articles'.SP.$lang.SP.$this->getRequest()->getControllerName();
        $this->view->lang = $lang;
        $novost = Articles::getInstance()->getArticleById($id);
        // $this->view->peoples = Peoples::getInstance()->fetchAll(null, 'priority DESC');
        ...
        if ($this->_request->isPost()) {
            ...
            if ($data['new']['name']!='' ) {
                ...
                Articles::getInstance()->editArticle($new,$id);
                // иконка
                $img_name = $_FILES['image_small']['name'];
                $img_source = $_FILES['image_small']['tmp_name'];
                $delete_img = $this->_getParam('delete_img_small');
                if ($img_name!='' && $img_source!='' && !$delete_img) {
                    $item = Articles::getInstance()->getArticleById($id);
                    $ext = @end(explode('.', $img_name));
                    $small_img = DIR_PUBLIC.'pics/articles/'.$id.'_small.'.$ext;
                    $big_img = DIR_PUBLIC.'pics/articles/'.$id.'_big.'.$ext;
                    if(copy($img_source, $small_img)) {
                    ...
                    $item = Articles::getInstance()->getArticleById($id);
                    @unlink(DIR_PUBLIC.'pics/articles/'.$item->small_img);
                    $item->small_img='';
                    $item->save();
                }
                // картинка
                ...
                if ($img_name!='' && $img_source!='' && !$delete_img) {
                    ...
                    $item = Articles::getInstance()->getArticleById($id);
                    @unlink(DIR_PUBLIC.'pics/articles/'.$item->big_img);
                    $item->big_img='';
                    $item->save();
                }

                $this->editMeta('articles', $id);
        ...
        $fck2 = $this->getFck('new[content]', '100%', '300');
        $this->view->fck_content = $fck2;
        $this->view->options = $this->getMeta('articles', $id);
    ...
    public function pubAction() {
        $lang = $this->getRequest()->getParam('lang','ru');
        $curModul = SP.'articles'.SP.$lang.SP.$this->getRequest()->getControllerName();
        if(!$this->_hasParam('id')) {
            $this->_redirect($curModul);
        }
        else {
            $id = (int)$this->getRequest()->getParam('id');
            Articles::getInstance()->pubArticle($id);
    ...
    public function unpubAction() {
        ...
        if(!$this->_hasParam('id')) {
            ...
            $id = (int)$this->getRequest()->getParam('id');
            Articles::getInstance()->unpubArticle($id);
            $sortSession = new Zend_Session_Namespace('sortSearch');
            $pageSession = new Zend_Session_Namespace('pageSearch');
    ...
    public function checkurlAction() {
        ...
        if ($url) {
            $item = Articles::getInstance()->fetchRow("url='$url' AND id!='$id'");
            if ($item!=null) {
                echo 'err';
            } else echo 'ok';
    ...
    public function deleteAction() {
        ...
        if(!$this->_hasParam('id')) {
            ...
            Articles::getInstance()->deleteArticle($id);
            $sortSession = new Zend_Session_Namespace('sortSearch');
            $pageSession = new Zend_Session_Namespace('pageSearch');
    ...
    public function setmainAction() {
        ...
        if(!$this->_hasParam('id')) {
            ...
            $id = (int)$this->getRequest()->getParam('id');
            Articles::getInstance()->setMainArticle($id);
            $sortSession = new Zend_Session_Namespace('sortSearch');
            $pageSession = new Zend_Session_Namespace('pageSearch');
    ...
    public function unsetmainAction() {
        ...
        if(!$this->_hasParam('id')) {
            ...
            $id = (int)$this->getRequest()->getParam('id');
            Articles::getInstance()->unsetMainArticle($id);
            $sortSession = new Zend_Session_Namespace('sortSearch');
            $pageSession = new Zend_Session_Namespace('pageSearch');
    ...
    public function activateAction() {
        ...
        $this->_redirect("/articles/$lang/admin_articles/");
    }
    ...
    private function editRoute($data) {
        ...
        if(!Router::getInstance()->replaceRoute($data, 'index', 'index', 'articles')) {
