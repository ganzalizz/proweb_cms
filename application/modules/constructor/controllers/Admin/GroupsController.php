...
class Constructor_Admin_GroupsController extends MainAdminController  {
	...
	public function init(){
		...
		$this->view->caption = 'Группы ингредиентов';
	...
	public function indexAction() {
		...
		if ($id){
			$this->layout->action_title = "Список групп ингредиентов";
                        ...
                        $this->view->total = count(Constructor_Groups::getInstance()->fetchAll());
                        $this->view->all = Constructor_Groups::getInstance()->fetchAll("id_type=$id", 'priority DESC', (int)$this->_onpage, (int)$this->_offset);
		} else {
		...
		if ($id){
			$item = Constructor_Groups::getInstance()->find($id)->current();
			$this->layout->action_title = "Редактировать группу ингредиентов";
		} else{
			$item = Constructor_Groups::getInstance()->fetchNew();
                        $this->layout->action_title = "Создать группу ингредиентов";
		}
	...
	public function activeAction(){
		if($this->_hasParam('id')){
			...
			$page = Constructor_Groups::getInstance()->find($id)->current();
			if (!is_null($page)){
