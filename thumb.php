...
if (file_exists($img)) {
 ...
 $cached_filename=md5($img.filemtime($img).$new_width.$new_height.$type).'.jpg';
 //$path_to_cache_file=_I_CACHE_PATH.substr($cached_filename, 0, 2);
 $path_to_cache_file = _I_CACHE_PATH.$new_width."x".$new_height;
 ...
 {
   case 0:
   	if (($new_width>=$image_width) && ($new_height>=$image_height) && $image_format==1)
   	{
